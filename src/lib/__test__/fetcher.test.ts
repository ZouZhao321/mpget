  ...
  it("解析搜索结果", async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          mockRes("https://weixin.sogou.com/weixin?query=AI", html),
        ),
    );
  ...
  it("反爬检测", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          mockRes("https://weixin.sogou.com/antispider/", "antispider"),
        ),
    );
  ...
  it("空结果", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          mockRes("https://weixin.sogou.com/weixin", "<html></html>"),
        ),
    );
    const r = await searchSogou("AI");
...
describe("resolveRealUrl", () => {
  ...
  it("提取真实URL", async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(mockRes("https://weixin.sogou.com/redirect", html)),
    );
...
describe("fetchArticleContent", () => {
  ...
  it("提取正文", async () => {
    ...
    expect(await fetchArticleContent("https://mp.weixin.qq.com/s/test")).toBe(
      "正文内容",
    );
  });
